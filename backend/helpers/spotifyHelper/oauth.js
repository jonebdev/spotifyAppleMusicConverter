...
module.exports = {
  ...
  async spotifyCallback(req, res){
    ...
    if (state === null || state !== storedState){
      const errorRedirectURL = new URL('/#').searchParams.append('error', 'state_mismatch')
      ...
      try{
        const getToken = await fetch.post('https://accounts.spotify.com/api/token', authOptions)
        ...
        const meRequest = await fetch.get('https://api.spotify.com/v1/me', options)
